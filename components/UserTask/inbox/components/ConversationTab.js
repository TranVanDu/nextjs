...
function ConversationTab(props) {
    const { firstLoading, listBookingConversation, config, itemActive, setItemActive } = props;
    return (
        <React.Fragment>
            {firstLoading ?
                <Row align="center">
                            ...
                            <Row>
                                <Col xs={0} md={24} lg={24} sm={0}>
                                    <List.Item key={item.id} onClick={() => setItemActive(item)} className={itemActive && itemActive.id == item.id ? 'inbox-item-selected' : ''}>
                                        <List.Item.Meta
                                            ...
                                            title={<div>
                                                <div className={itemActive && itemActive.id == item.id ? 'text-color-primary' : ''}>{`${item.partner[0].partner_firstname} ${item.partner[0].partner_lastname}`}</div>
                                            </div>}
                                            description={<div>
                                ...
                                <Col xs={24} md={0} lg={0} sm={24}>
                                    <List.Item key={item.id} onClick={() => setItemActive(item)} className={itemActive && itemActive.id == item.id ? 'inbox-item-selected pointer' : 'pointer'}>
                                        ...
                                        <div className={itemActive && itemActive.id == item.id ? 'text-color-primary mr-2 text-center' : 'mr-2 text-center'}>{`${item.partner[0].partner_firstname} ${item.partner[0].partner_lastname}`}</div>
                                    </List.Item>
                                </Col>
                            </Row>
                        )
