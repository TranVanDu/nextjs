...
const TransportSearchBox = (props) => {
    const { t } = props;
    const [form] = Form.useForm();
    const windowSize = useWindowSize();

    const [isOnward, setIsOnward] = useState(true);
    const [place, setPlace] = useState(null);
    const [airport, setAirport] = useState(null);
    const [passenger, setPassenger] = useState(2);
    const [pickDate, setPickDate] = useState({ pickDateStr: null, pickDateMt: null });
    const [pickTime, setPickTime] = useState({ pickTimeStr: null, pickTimeMt: null });
    ...
    const setDataPlace = async (place) => {
        ...
        setPlace({ place_id: district ? district.id : null, sub_address: place.value, address_detail: place.formatted_address })
    }
    ...
    const onSearch = (values) => {
        ...
        var dataSearch = {
            ...
            province_latitude: airport.province_latitude,
            province_longitude: airport.province_longitude,
        ...
        Router.push({ pathname: '/transport/list', query: dataSearch }, undefined);
    }
...
export default withTranslation('home')(TransportSearchBox);