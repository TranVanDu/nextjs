...
export const getConversationDetail = (id, cid) => dispatch => {
    return new Promise((resolve, reject) => {
        api.get(`/chat/conversation_detail/${id}?cid=${cid}`).then(res => {
            resolve(res.data);
            dispatch({ type: GET_CVS_DETAIL, payload: res.data.data })
        })
            .catch(err => {
                reject(err)
            })
